...
var Dyatlov = function (element_id, toolkit, config) {
  var map_module;
  if (toolkit) map_module = this.maps[toolkit];
  if (map_module == null) map_module = this.detect_toolkit();
...
Dyatlov.prototype = {
  ...
  RX:
    ((C = function (data) {
      ...
      if (this.snr == null)
        this.snr =
          typeof snr == "object" && snr && snr[this.raw.id] != null
            ? Number(snr[this.raw.id])
            : null;
      this.blocked =
        typeof blocked == "object" && blocked && blocked[this.raw.id] != null
          ? Number(blocked[this.raw.id])
          : null;
    ...
    (C.prototype = {
      ...
      parse_snr: function () {
        ...
        if (
          /* window.location.hash == '#snrhf' && */ !(
            Number.isNaN(snr) || snr == 0
          )
        )
          return snr;
        snr = Number.parseInt(scores[0], 10);
        if (!(Number.isNaN(snr) || snr == 0)) return snr;
        return null;
      },
      ...
      liveness_age: function () {
        ...
        if (this.parsed.updated == null) return 0;
      ...
      wideband: function () {
        return (
          this.parsed.bandwidth >= 5000000 &&
          !(this.blocked && this.blocked > 0.5)
        );
      },
      ...
      availability: function () {
        ...
        if (users.current == null || users.max == null) return null;
      ...
      precedence: function () {
        var precedence = 15;
        if (this.snr != null) precedence = this.snr;
        ...
        // blocked spectrum
        if (this.blocked) precedence *= 1 - this.blocked;
        ...
        if (this.offline()) precedence /= 100;
...
*/
      ...
      validate: function () {
        return this.raw.name && this.raw.url && this.recent();
      },
      // Content of title-attribute marker tooltip
      marker_title: function () {
        var lines = [];
        lines.push(this.raw.name);
        if (this.raw.sdr_hw) lines.push("Setup: " + this.raw.sdr_hw);
        if (this.raw.antenna) lines.push("Antenna: " + this.raw.antenna);
        ...
        } else lines.push("Currently offline (check receiver for details)");

        if (this.snr != null)
          lines.push("S/N score: " + this.snr.toFixed(0) + " dB");
        if (this.gps())
          lines.push(
            "GPS clock available: " + this.parsed.gps_fpm + " fixes/min",
          );
        if (this.blocked)
          lines.push(
            "Blocked spectrum: " + Number(this.blocked * 100).toFixed(1) + "%",
          );
      ...
      marker_color: function () {
        if (this.offline()) return "9067FD"; // Purple

        var avail = this.availability();
        if (avail != null && !avail) return "FFFF6E"; // Yellow

        if (
          window.location.hash == "#all" &&
          (!this.wideband() || !this.shortwave())
        )
          return "0080FF"; // TBD

        if (avail == null) return "00E74C"; // Green

        var q = this.quality();
        if (q == null) return "807567"; // Gray
        ...
        return (
          ...
          encodeURIComponent(
            '<?xml version="1.0" encoding="UTF-8" standalone="no"?><svg xmlns="http://www.w3.org/2000/svg" width="21" height="34"><path d="M 19.5,9.5469494 C 19.5,12.03223 18.097139,13.965539 16.863961,15.91091 8.422781,29.22709 12.109244,35.147897 10.6875,33.109449 10.077219,32.234449 12.639719,27.66459 4.136039,15.91091 2.785922,14.044796 1.5,12.03223 1.5,9.5469494 c 0,-4.9705629 4.029437,-9.00000031 9,-9.00000031 4.970563,-1e-7 9,4.02943731 9,9.00000031 z" style="fill:#' +
              this.marker_color() +
              ';stroke:#000000;stroke-width:1.3;" /><circle r="1.5" cy="10" cx="10.5" style="fill:#000000;" /></svg>',
          )
        );
      },
      ...
      bubble_HTML: function () {
        return (
          '<a href="' +
          this.xml_escape(this.raw.url) +
          '" target="_blank" style="color:teal;font-weight:bold;text-decoration:none" title="' +
          this.xml_escape(this.title) +
          '">' +
          this.xml_escape(this.raw.name) +
          "</a>"
        );
      },
    }),
  ...
  maps: {
    ...
    Leaflet:
      ...
      ((C = function (element_id, config) {
        ...
        if (tslayers.length > 0) tslayers[0].layer.addTo(this.map);
    ...
    Builtin:
      ...
      (C.prototype = {
        ...
        add_marker: function (rx, coords) {
          ...
          li.setAttribute(
            "style",
            "list-style-image: url('" + rx.marker_icon() + "');",
          );
  ...
  detect_toolkit: function (element_id) {
    ...
    if (typeof L == "object" && L && typeof L.map == "function") {
  ...
  distinct_marker: function (coords) {
    ...
    for (var tries = 10; ; tries--) {
      ...
      var key =
        Math.round(coords.lat / gran) + "," + Math.round(coords.lng / gran);
